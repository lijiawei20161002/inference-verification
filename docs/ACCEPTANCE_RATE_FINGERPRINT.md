...
Two pieces in `ivgym/spec_decode.py`:

* **`trace_features(trace)`** — reads `accept_rate` straight off the reported
  trace: `mean([s.accepted for s in trace.steps])`. No forward pass. (It also
  computes `mean_entropy` and `mean_surprisal` for the generic baseline
  fingerprint.)
* **`ReferenceFingerprint`** — `.fit(honest_traces)` records the per-feature
  mean/std of a trusted honest run (the external anchor, computed once, offline).
  `.score(trace)` returns the standardized distance `|value − mean| / std` per
  feature. Detection = threshold on that z-distance.
...
That's it — the power comes entirely from `q` being trustworthy.
...
* **Panel A — mechanism.** As quantization σ grows, the per-trace acceptance-rate
...
* **Panel B — it scales, and leads.** Detection AUC vs σ: the acceptance-rate
  ...
  while the recompute spot-check (blue) is flat at 1.0. Note both fingerprints
...
* it does not dominate `target_spotcheck` (exact recompute, AUC 1.0 throughout).

What it buys is a **shrink in how often the exact recompute must fire** — the
draft-anchored acceptance-rate test is the one no-recompute algorithm the SD trace
adds *over* generic black-box statistics, because matching entropy does not
restore `TV(p̂, q)`.
...
python tests/test_spec_decode.py                     # 14 spec-decode tests
```
