...
Exercises `harness.verify_selective` -- the proxy-triaged cost-aware tier -- through
the SAME HFGPUBackend / attacks / defenses / evaluate pipeline as `exp_gpu.py`, so
it shows the tier is a first-class citizen alongside `verify` (full recompute) and
`io_verify` (no recompute). For each recompute budget it reports the realized
recompute ratio and the detection AUC (honest vs attack), calibrating the honest
reference with the SAME budget.
...
def main():
    ...
    # generate_dataset, so every backend read (verify / proxy tie / selective) must
    ...
    h_tri = harness.proxy_tie_scores(backend, honest, spec)
    h_sel = {b: harness.verify_selective(backend, honest, spec, td, b, triage=h_tri)
             for b in BUDGETS}

    attack = harness.generate_dataset(backend, attacks.get(ATTACK), spec, N, T)
    attack_full = harness.verify(backend, attack, spec, [td])
    a_tri = harness.proxy_tie_scores(backend, attack, spec)
    a_sel = {b: harness.verify_selective(backend, attack, spec, td, b, triage=a_tri)
             for b in BUDGETS}
    ...
    print("  " + "-" * 46)
    for bud in BUDGETS:
        h_scores, r_h = h_sel[bud]
        a_scores, r_a = a_sel[bud]
